...
### 发布工程（v4.6.0 随版落地）
- **自更新源切换到本仓库**（`20090106-520/Deepseek-Harness-EAC`）：客户端
  检查更新（GitHub → Gitee 双源）与「反馈建议」链接全部指向本仓库；应用内
  更新从此拉取本仓库 Releases（SHA-256 强校验不变）。
- **内置 dsh 内核升级**：`@deepseek-ai/dsh` 全家 19 个官方包
  `0.1.0-rc.7 → 0.1.1-rc.2`（npm 最新），533 项内核测试全绿后入包。
- **代码签名接入准备**：新增《代码签名政策》页（`SIGNING_POLICY.md`，满足
  SignPath Foundation 免费开源证书申请要求）、签名指南与构建后补签脚本
  （`docs/CODE_SIGNING.md` / `scripts/sign-release.ps1`）；凭据到位即自动
  签名，未配置时不影响出包。
...
- chrome 栏 ⋯ 菜单与托盘菜单新增「反馈建议…」，直达 GitHub Issues
  （`https://github.com/20090106-520/Deepseek-Harness-EAC/issues`）；
