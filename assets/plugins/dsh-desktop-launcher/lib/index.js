...
async function probeDsh(platform, run, dshCommand) {
	...
	try {
		return (platform === "win32" ? await run("where", [dshCommand]) : await run("sh", ["-lc", `command -v ${dshCommand}`])).code === 0;
	} catch {
		return false;
	}
}
	...
	return { routes: [{
		...
		handler: async (req, res) => {
			if (!isLoopbackRequest(req)) {
				writeJson$1(res, 403, { error: "forbidden: loopback-only" });
				return;
			}
			if ((req.method ?? "GET") !== "POST") {
				writeJson$1(res, 405, { error: `method not allowed: ${req.method}` });
				return;
			}
			try {
				writeJson$1(res, 200, { result: await createDesktopShortcut(deps) });
			} catch (error) {
				writeJson$1(res, 500, { error: error instanceof Error ? error.message : String(error) });
			}
		}
	}] };
}
...
function makeShutdownRoute(deps) {
	...
	return {
		...
		handler: async (req, res) => {
			...
			schedule(() => deps.requestExit(0), 80);
		}
	};
}
//#endregion
...
* (dsh-web-ui-all) namespaces every child row id (web-ui-*), so
