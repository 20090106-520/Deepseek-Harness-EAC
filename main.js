...
function registerChromeIpc() {
  ...
  ipcMain.handle('chrome:menu', async (event, { action, value } = {}) => {
    ...
    switch (action) {
      ...
      case 'feedback': shell.openExternal('https://github.com/20090106-520/Deepseek-Harness-EAC/issues'); break;
...
function createTray() {
  ...
  try {
    ...
    const menu = Menu.buildFromTemplate([
      ...
      { type: 'separator' },
      { label: '反馈建议…', click: () => { showMainWindow(); shell.openExternal('https://github.com/20090106-520/Deepseek-Harness-EAC/issues'); } },
      { type: 'separator' },
...
const COMPANION_PLUGINS = [
  ...
  // Git 分支选择器与提交图谱（dsh-git-graph，用户建议）：composer 上方注入
  // 当前分支 chip，点击弹出本地分支列表（可切换/新建）；Shift+点击打开
  // 最近 200 次提交的 ASCII 图谱面板；SSE 监听工作区变更自动刷新。
  { id: 'git-graph', name: 'dsh-git-graph', dir: 'dsh-git-graph' },
  // 设置页侧边栏分组 + 常规页高级选项折叠（dsh-settings-groups，用户建议）：
  // 侧边栏按关键词分「普通/高级」两组，高级默认收起；常规页低频选项行
  // 收进底部「高级选项」折叠组；展开状态 localStorage 持久化。
  { id: 'settings-groups', name: 'dsh-settings-groups', dir: 'dsh-settings-groups' },
  // 设置页插件市场（dsh-plugin-marketplace，用户建议）：浏览/搜索 npm 上的
  // dsh 插件并一键安装到 web profile；host 半边以 Typert Remote 驱动 npm；
  // 设置页新增「插件市场」标签页。
  { id: 'plugin-marketplace', name: '@deepseek-ai/dsh-plugin-marketplace', dir: 'dsh-plugin-marketplace' },
  // 对话恢复（dsh-chat-recovery，用户建议）：编辑最后一条用户消息并重新发送，
  // 失败轮次自动重试（指数退避 1s/2s/4s/8s/16s），支持手动重试。纯客户端实现，
  // host 半边为 no-op，通过 conversation.chat.turnTail + conversation.input.dock 注入。
  { id: 'chat-recovery', name: '@linxin666/dsh-chat-recovery', dir: 'dsh-chat-recovery' },
  // 桌面启动器 + 一键关机（dsh-desktop-launcher，用户建议）：创建/管理桌面图标、
  // 页面右下角浮动关机按钮；host 半边注册 /api/dsh-desktop-launcher/* 路由（loopback 仅限），
  // client 半边注入设置卡片与浮动按钮。
  { id: 'desktop-launcher', name: '@linxin666/dsh-desktop-launcher', dir: 'dsh-desktop-launcher' },
];
